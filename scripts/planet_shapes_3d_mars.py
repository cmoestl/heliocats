...
mav= pickle.load( open( "DATACAT/MAVEN_2014to2018_MSO_removed.p", "rb" ) )
print('how much data is not NAN in the removed dataset?')

#cut off orbit insertion
mav=mav[10000:np.size(mav)]
...
#print( 'read MAVEN')
#get insitu data
mav= pickle.load( open( "DATACAT/MAVEN_2014to2018_MSO.p", "rb" ) )

#cut off orbit insertion
mav=mav[10000:np.size(mav)]

print('how much data is not NAN in the non-removed dataset?')
a=np.isnan(mav.btot) 
b=np.where(a == False)
print(np.size(b)/np.size(mav.btot)*100)
...
xx=np.linspace(min(mav.x),max(mav.x),71)
yy=np.linspace(min(mav.y),max(mav.y),71)
zz=np.linspace(min(mav.z),max(mav.z),71)

btotaverage=np.nanmean(mav.btot)
background_wind=4
...
if compute_grid_xy > 0:
 ...
 print('start loop')
 insidez=np.where(np.logical_and(mav.z < 1.5, mav.z > -1.5))
 ...
 for i in range(np.size(xx)-1):
   ...
   for j in range(np.size(yy)-1): 
     ...
     insidex=np.where(np.logical_and(mav.x[insidez] < xx[i+1],mav.x[insidez] > xx[i]))
     insidey=np.where(np.logical_and(mav.y[insidez] < yy[j+1],mav.y[insidez] > yy[j]))
     #make an array of only those points that are inside the cell
     active=np.intersect1d(insidex,insidey)
     #minimum 10 datapoints
     if np.size(np.where(np.isnan(mav.btot[insidez][active]) == False)) > 10: ZZ[i,j]=np.nanmedian(mav.btot[insidez][active]) 
 print('end loop')
 pickle.dump([XX,YY,ZZ], open( "plots/planets/mav_XY_plane_non_removed.p", "wb" ) )
 
 
if compute_grid_xy == 0: XX, YY, ZZ = pickle.load( open( "plots/planets/mav_XY_plane_non_removed.p", "rb" ) )
...
#plt.imshow(np.log(ZZ.transpose()/np.nanmean(mav.btot)), cmap='inferno', interpolation='nearest',extent=[min(xx), max(xx), min(yy),max(yy)])
#plt.imshow(ZZ.transpose()/btotaverage, cmap='inferno', interpolation='nearest',extent=[min(mav.x), max(mav.x), min(mav.y),max(mav.y)])
...
plt.ylabel('Y MSO [Mars radii]')
plt.xlabel('X MSO [Mars radii]')
plt.title('Total magnetic field         MAVEN at Mars November 2014 - January 2018')
plt.figtext(0.02,0.01,r'Data from MAG instrument: Connerney et al. (2015). Shapes from Edberg et al. (2008). $C. M\ddot{o}stl$', fontsize=8, ha='left')
...
#used for data processing rbow[i]=( (0.55+0.12)  +(2.1 +0.09)/ (1.0+(1.05-0.04)*cos(sza[i])))*rmars
Rbs= (0.55+0.12)  +(2.1+0.09)/ (1+(1.05-0.04)*np.cos(SZA))
...
#normal shape
Rbs= (0.55)  +(2.1)/ (1+(1.05)*np.cos(SZA))
xbs,ybs=pol2cart(Rbs,SZA)
plt.plot(xbs,ybs, color='black',linewidth=2, linestyle='--')
...
plt.show()
plt.savefig('plots/planets/Mars_shape_XY.pdf', format='pdf', dpi=300)
plt.savefig('plots/planets/Mars_shape_XY.png', format='png', dpi=300)
